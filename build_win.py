...
def main(chapters=[], epub=False, pdf=False, html=False, mobi=False, pandoc_epub=False):
    ...
    process_chapters = [
        chapter_markdown for chapter_markdown in chapter_markdowns
    ]
    ...
    run('cmd /C copy /Y minutiae\\pdf\\ tex')
    ...
    if pdf:
        for imgpath in image_paths:
            ...
            dirname = imgpath.rpartition('\\')[-1]
            os.mkdir("tex/"+dirname)
            run('copy /Y {imgpath} tex\\{dirname}'.format(imgpath=imgpath, dirname=dirname))
    ...
    if epub:
        for imgpath in image_paths:
            run('copy /Y {imgpath} epub\\'.format(imgpath=imgpath))
...
def pandoc_cmd(chapter_markdown):
    ...
    result = subprocess.call(_pandoc_cmd(new_chapter_markdown), shell = True)
    if result != 0:
        print "Error in processing of {0}".format(new_chapter_markdown)

    result2 = subprocess.call(postprocessor_command(basename), shell=True)
    return result2


def run(cmd):
    print cmd
    result = subprocess.call(cmd, shell=True, stderr=subprocess.STDOUT)
    return result
